...
export const OperatorStoreSaleDetails = () => {
  const { id } = useParams();
    ...
    useEffect(() => {
        if (singleSale) {
            setFormData(singleSale.products);
            setOrderDate(singleSale.orderDate);
            setNote(singleSale.note);
            setOperatorInfo({
                id: singleSale.operatorId,
                firstName: singleSale.operatorFirstName,
            });
        }
    }, [singleSale]);
    ...
    return isLoading ? (
        ...
        <>
            <Breadcrumb pageName="Requested Details" />
